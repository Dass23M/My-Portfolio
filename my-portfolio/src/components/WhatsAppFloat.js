...
export default function WhatsAppFloat() {
  ...
  const phoneNumber = "94771234567"; // Your Sri Lankan number with country code
    ...
    <>
      ...
      <div className={`fixed bottom-6 right-6 z-50 transition-all duration-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-16 opacity-0'}`}>
        <button
          ...
          className={`group relative bg-green-500 hover:bg-green-600 text-white p-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 active:scale-95 ${showPulse ? 'animate-pulse' : ''}`}
        ...
        >
          ...
          <div className="relative flex items-center justify-center">
            {isOpen ? (
              <XMarkIcon className="w-8 h-8 transition-transform duration-300 rotate-180" />
            ) : (
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 animate-fade-in-up">
          <div className="bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl border border-gray-200 min-w-80 max-w-sm">
            {/* Header */}
            <div className="p-6 border-b border-gray-100">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <div>
                  <h3 className="font-bold text-black">Methmal</h3>
            ...
            <div className="p-4">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <span className="text-sm font-semibold text-gray-900">Quick message options:</span>
              </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {quickMessages.map((message, index) => (
                  <button
                    ...
                    className="w-full p-3 text-left text-sm text-gray-700 hover:bg-green-50 rounded-xl border border-gray-100 hover:border-green-200 transition-all duration-300 hover:shadow-sm group"
                  >
                    <div className="flex items-center space-x-3">
                      <span className="text-lg">{message.icon}</span>
                      <div className="flex-1">
                        <div className="font-medium text-gray-900 group-hover:text-green-700 transition-colors duration-300">
                          {message.text}
                        </div>
                      ...
                      <PaperAirplaneIcon className="w-4 h-4 text-gray-400 group-hover:text-green-500 transition-colors duration-300" />
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <button
                  ...
                  className="w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white p-3 rounded-xl font-semibold text-sm transition-all duration-300 hover:shadow-lg hover:scale-105 active:scale-95 flex items-center justify-center space-x-2"
                >
